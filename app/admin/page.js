...
export default function AdminDashboard() {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/estimate-submissions?${queryParams.toString()}`);
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        if (data.message?.includes('Database table not found') || 
      ...
      setSubmissionsError('An error occurred while fetching submissions. Check console for details.');
      setDbConfigured(false);
    } finally {
  ...
  const viewSubmissionDetails = async (id) => {
    try {
      ...
      if (response.ok) {
        ...
        setSubmissionsError(data.message || 'Failed to fetch submission details');
      }
    } catch (err) {
      setSubmissionsError('An error occurred while fetching submission details');
      console.error(err);
    } finally {
      setSubmissionsLoading(false);
    }
  ...
  const updateSubmissionStatus = async (id, status) => {
    try {
      ...
      if (response.ok) {
        ...
        setSubmissionsError(data.message || 'Failed to update submission status');
      }
    } catch (err) {
      ...
      <div className={styles.section}>
        ...
        {!dbConfigured ? (
          <div className={styles.setupMessage}>
            ...
            <p>The estimate submissions database tables don't appear to be set up correctly.</p>
            <p>Please run the database setup script to create the necessary tables:</p>
          ...
          <>
            ...
            <form onSubmit={applyFilters} className={styles.form}>
              ...
              <div className={styles.formFields}>
                ...
                <div className={styles.formGroup}>
                  <label htmlFor="searchTerm">Search (Name, Email, Business)</label>
                  <input
                    type="text"
                    id="searchTerm"
                    name="searchTerm"
                ...
                <>
                  ...
                  <div className={styles.pagination}>
                    ...
                    <span className={styles.pageInfo}>
                      Page {submissionPage} of {Math.ceil(totalSubmissions / submissionsPerPage)}
                    </span>
                    
                    <button 
                      onClick={() => handlePageChange(submissionPage + 1)}
                      disabled={submissionPage >= Math.ceil(totalSubmissions / submissionsPerPage)}
                      className={styles.paginationButton}
                    >
                      Next
                    </button>
            ...
            <div className={styles.modalContent}>
              <div className={styles.detailSection}>
                ...
                <div className={styles.statusActions}>
                  <span className={`${styles.statusBadge} ${styles[currentSubmission.status]}`}>
                  ...
                  <div className={styles.statusControls}>
                    ...
                    <select
                      id="changeStatus"
                      onChange={(e) => updateSubmissionStatus(currentSubmission.submission_id, e.target.value)}
              ...
              <div className={styles.detailSection}>
                ...
                <div className={styles.detailGrid}>
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>{currentSubmission.name}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Business:</span>
                    <span className={styles.detailValue}>{currentSubmission.business_name || 'N/A'}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Email:</span>
                    <span className={styles.detailValue}>{currentSubmission.email}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Phone:</span>
                    <span className={styles.detailValue}>{currentSubmission.phone}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Old Website:</span>
                    <span className={styles.detailValue}>
                      {currentSubmission.old_website ? (
                        <a href={currentSubmission.old_website} target="_blank" rel="noopener noreferrer">
                          {currentSubmission.old_website}
                        </a>
                      ) : 'N/A'}
                    </span>
                  ...
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>{formatDate(currentSubmission.submitted_at)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={styles.detailSection}>
                ...
                <div className={styles.detailGrid}>
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>
                      {currentSubmission.website_design_approach.charAt(0).toUpperCase() + 
                       currentSubmission.website_design_approach.slice(1)}
                    </span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Page Count:</span>
                    <span className={styles.detailValue}>{currentSubmission.page_count}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Mobile First:</span>
                    <span className={styles.detailValue}>{currentSubmission.mobile_first ? 'Yes' : 'No'}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Convert Old Website:</span>
                    <span className={styles.detailValue}>{currentSubmission.convert_old_website ? 'Yes' : 'No'}</span>
                  </div>
                </div>
                
                <div className={styles.fullWidthDetail}>
                  <span className={styles.detailLabel}>Description:</span>
                  <p className={styles.detailValue}>{currentSubmission.website_description}</p>
                </div>
              </div>
              ...
              <div className={styles.detailSection}>
                ...
                <div className={styles.detailGrid}>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Frontend Technologies:</span>
                    <span className={styles.detailValue}>
                      {currentSubmission.frontend_technologies && 
                       currentSubmission.frontend_technologies.length > 0 ? 
                       currentSubmission.frontend_technologies.join(', ') : 'N/A'}
                    </span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Backend:</span>
                    <span className={styles.detailValue}>{currentSubmission.backend}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Database:</span>
                    <span className={styles.detailValue}>{currentSubmission.database_type}</span>
                  </div>
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>
                      {currentSubmission.integrations && 
                       ...
                       currentSubmission.integrations.join(', ') : 'None'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className={styles.detailSection}>
                ...
                <div className={styles.detailGrid}>
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>{currentSubmission.budget}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Timeline:</span>
                    <span className={styles.detailValue}>{currentSubmission.timeline}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Delivery Timeframe:</span>
                    <span className={styles.detailValue}>{currentSubmission.delivery_timeframe}</span>
                  </div>
                </div>
                ...
                {currentSubmission.additional_info && (
                  <div className={styles.fullWidthDetail}>
                    ...
                    <p className={styles.detailValue}>{currentSubmission.additional_info}</p>
                  </div>
                )}
              </div>
                ...
                <div className={styles.detailSection}>
                  ...
                  <div className={styles.detailGrid}>
                    ...
                    <div className={styles.detailItem}>
                      ...
                      <span className={styles.detailValue}>{currentSubmission.payment_gateway_type || 'Not specified'}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {currentSubmission.google_ads_enabled && (
                <div className={styles.detailSection}>
                  ...
                  <div className={styles.detailGrid}>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Budget:</span>
                      <span className={styles.detailValue}>{currentSubmission.google_ads_budget || 'Not specified'}</span>
                    </div>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Campaign Type:</span>
                      <span className={styles.detailValue}>{currentSubmission.google_ads_campaign_type || 'Not specified'}</span>
                    </div>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Location:</span>
                      <span className={styles.detailValue}>{currentSubmission.google_ads_location || 'Not specified'}</span>
                    </div>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Has Existing Account:</span>
                      <span className={styles.detailValue}>{currentSubmission.google_ads_has_account ? 'Yes' : 'No'}</span>
                    </div>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Needs Management:</span>
                      <span className={styles.detailValue}>{currentSubmission.google_ads_need_management ? 'Yes' : 'No'}</span>
                    </div>
                  </div>
                  ...
                  {currentSubmission.google_ads_keywords && (
                    <div className={styles.fullWidthDetail}>
                      ...
                      <p className={styles.detailValue}>{currentSubmission.google_ads_keywords}</p>
                    </div>
                  )}
                </div>
              )}
