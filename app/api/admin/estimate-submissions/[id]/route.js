...
export async function GET(request, { params }) {
  try {
    ...
    const submissions = await executeQuery({
      query: 'SELECT * FROM vw_estimate_submissions_complete WHERE submission_id = ?',
      values: [id]
    });
    ...
    try {
      if (submission.frontend_technologies) {
        submission.frontend_technologies = JSON.parse(submission.frontend_technologies);
      } else {
    ...
    submission.google_ads_need_management = !!submission.google_ads_need_management;
...
export async function PATCH(request, { params }) {
  try {
    ...
    const validStatuses = ['pending', 'in_review', 'quoted', 'accepted', 'rejected'];
